...
//        StreamExecutionEnvironment env = ExecutionEnvUtil.prepare(parameterTool);
        ...
        DataStream<Tuple4<String, String, String, Integer>> result = detail.keyBy(new KeySelector<Tuple2<UserEvent, Integer>, String>() {
            @Override
            public String getKey(Tuple2<UserEvent, Integer> value) throws Exception {
                ...
                return value.f0.getPageName() + "&&" + time.toLocalDate().toString();
            }
        }).window(TumblingEventTimeWindows.of(Time.days(1), Time.hours(-8)))
                .trigger(ContinuousProcessingTimeTrigger.of(Time.seconds(10)))
//                .trigger(CountTrigger.of(1))
                .evictor(TimeEvictor.of(Time.seconds(0), true))    //每次窗口计算剔除已经消息过数据
                .process(new ProcessWindowFunction<Tuple2<UserEvent, Integer>, Tuple4<String, String, String, Integer>, String, TimeWindow>() {

                    // 防止uvCountState过大，可以用BoomFilter
                    ...
                    @Override
                    public void process(String s, Context context, Iterable<Tuple2<UserEvent, Integer>> iterable, Collector<Tuple4<String, String, String, Integer>> collector) throws Exception {
                        Integer pv = 0;
                        ...
                        String[] keys = s.split("&&");
                        collector.collect(Tuple4.of(keys[0], keys[1], "uv", uv));
                        collector.collect(Tuple4.of(keys[0], keys[1], "pv", pvCountState.value()));
                    }
                });
        ...
        result.addSink(new FlinkKafkaProducer011<Tuple4<String, String, String, Integer>>(
                "localhost:9092", "fink-point-sink", new SerializationSchema<Tuple4<String, String, String, Integer>>() {
            @Override
            public byte[] serialize(Tuple4<String, String, String, Integer> tuple2) {
                try {
