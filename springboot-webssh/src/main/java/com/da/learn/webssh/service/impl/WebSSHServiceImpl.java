package com.da.learn.webssh.service.impl;

import com.da.learn.webssh.constant.ConstantPool;
import com.da.learn.webssh.pojo.SSHConnectInfo;
import com.da.learn.webssh.pojo.WebSSHData;
...
public class WebSSHServiceImpl implements WebSSHService {
    ...
    /**
     * 初始化连接，实际上就是将我们所需要的连接信息，保存在一个Map中，
     * 这里并不进行任何的真实连接操作。为什么这里不直接进行连接？
     * 因为这里前端只是连接上了WebSocket，但是我们还需要前端给我们发来linux终端的用户名和密码，
     * 没有这些信息，我们是无法进行连接的
     *
     * @param session
     */
    ...
    /**
     * 处理客户端发送的数据
     * 1 如果客户端发来的是终端的用户名和密码等信息，那么我们进行终端的连接。
     * 2 如果客户端发来的是操作终端的命令，那么我们就直接转发到终端并且获取终端的执行结果。
     *
     * @param buffer
     * @param session
     */
